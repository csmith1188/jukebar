...
const trackCheckInterval = setInterval(async () => {
    ...
    try {
        ...
        if (!lastTrack || currentTrack.id !== lastTrack.id) {
            ...
            const progressTimeout = setTimeout(() => progressController.abort(), 30000);
...
function setJukepix(enabled) {
    ...
    if(jukepixEnabled) {
        ...
        setTimeout(() => {
            ...
            const msgTimeout = setTimeout(() => msgController.abort(), 30000);
        ...
        setTimeout(() => {
            ...
            const clearTimeout2 = setTimeout(() => clearController.abort(), 30000);
